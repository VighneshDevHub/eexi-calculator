"""
calculators/eexi/eexi_core.py
------------
Core EEXI attained / required calculation and compliance check.
"""

from ..utils.ship_params import SHIP_PARAMS, get_capacity, get_reduction_factor
...
def check_compliance(attained: float, required: float) -> dict:
    ...
    """
    status = "COMPLIANT" if attained <= required else "NON_COMPLIANT"
    margin = ((required - attained) / required) * 100.0 if required > 0 else 0.0
    return {
        "status": status,
