...
"""
from ..utils.ship_params import get_cf, get_capacity, SHIP_PARAMS
from .emissions import calc_pme, calc_me_emissions, calc_ae_emissions, calc_total_numerator, get_default_pae
from .eexi_core import calc_attained_eexi, calc_required_eexi, check_compliance
...
def calculate_eexi(data: dict) -> dict:
    ...
    # IMPORTANT: Reference line for container ships uses 100% DWT, 
    # while attained EEXI uses 70% DWT.
    ...
    epl_data = calc_epl(
        ...
        cf_ae=cf_ae,
        sfc_ae=sfc_ae,
        f_i=f_i, f_w=f_w, f_c=f_c, f_l=f_l, f_m=f_m,
        ship_type=ship_type,
        p_me_original=pme,
        n_exponent=n_exponent
    )
    
    # Calculate EPL percentage if possible
    if epl_data.get('epl_possible'):
        epl_data['reduction_pct'] = calc_epl_percentage(epl_data['mcr_lim'], mcr)
    ...
    return {
        ...
        'compliant': compliance.get('compliant', attained <= required),
        'epl': epl_data,
        'pme': round(pme, 1),
        'pae': round(pae, 1),
        'capacity': capacity,
